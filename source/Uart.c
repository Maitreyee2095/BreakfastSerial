...
int __sys_write(int handle, char *buf, int size)
{
	...
	while (*buf != '\0') { // copy characters up to null terminator
		while(cb_Full(&TxQ)) {
				; // Wait for the space to openup
			}
		if((cb_Enqueue(&TxQ, buf,1))==1)//check if the byte is enqueued
		{
			buf++; //increment pointer to next character
		}
	}
...
void UART0_IRQHandler(void) {
	...
	if (UART0->S1 & UART0_S1_RDRF_MASK) {
		...
		ch = UART0->D;
		cb_Enqueue(&RxQ, &ch ,1);//enqueue the character to receiver buffer
