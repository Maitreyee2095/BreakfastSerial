...
void cb_init(cb_t * c) {
	...
	c->Full = false;
	c->capacity = MAX;		//initialize buffer to max size
...
bool cb_Full(cb_t * c){
	assert(c);
	return (cb_Length(c) == MAX);// full condition return length as max//this code was inspired from dean's code
}
...
size_t cb_Length(cb_t * c){
	...
	if(c->Full){	//if circular buffer is full
		value = MAX; //update length value to max
	}
	else if(c->write >= c->read){
		value = c->write - c->read; //if write >= read update length value to write -read
	}
	else {
		value = MAX - (c->read - c->write);//if read >= write update length value to Max-(read-write) condition after write wraps around
...
size_t cb_Enqueue(cb_t * c, const void *buffer , size_t nbyte) {
	...
	if(cb_Empty(c)) {			    		//if buffer is empty
		...
		if(nbyte == MAX) {
			//if input nbyte length is equal to max length
			length1 = MAX;
	...
	{
		length1 = min(nbyte, MAX - c->write); //check how much bytes are left by MAX-c->wrtie and compare number of bytes to enqueue and bytes to copy
		...
		if (c->write < MAX) //check if buffer has still some space
		{
			ENDCRITICAL(masking);
...
size_t cb_Dequeue(cb_t * c, void *buffer , size_t nbyte) {
	...
	length1 = min(nbyte, MAX - c->read); //check which is minimum no of bytes to read or bytes to read on buffer
	...
	if(c->read < MAX) //check if read pointer has reached the last element
	{
		ENDCRITICAL(masking);
		return length1 + length2;
	}
